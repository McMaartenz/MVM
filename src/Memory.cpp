...
Memory::Memory(uint32_t memory_size) : memory_size(memory_size) {
...
void Memory::read_buffer(uint32_t address, uint8_t* buffer, uint32_t count) {
	// TODO: Apply MMU logic (How would this be done? e.g. in terms of optimization to still get great throughput)]
	std::copy_n(data.begin() + address, count, buffer);
	std::copy_n(buffer, count, data.begin() + address);
}
